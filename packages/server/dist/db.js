...
const DB_PATH = path.join(__dirname, '..', 'data', 'clawhq.db');
...
try {
    db.exec(`ALTER TABLE meetings ADD COLUMN parent_meeting_id TEXT`);
}
catch { /* exists */ }
try {
    db.exec(`ALTER TABLE meetings ADD COLUMN source_meeting_id TEXT`);
}
catch { /* exists */ }
try {
    db.exec(`ALTER TABLE meetings ADD COLUMN source_candidates TEXT`);
}
catch { /* exists */ }
try {
    db.exec(`ALTER TABLE meetings ADD COLUMN decision_packet TEXT`);
}
catch { /* exists */ }
