...
import { useT, useLang, t as tStatic } from '../i18n';
...
function getWindowOptions(): Array<{ key: MonitoringWindow; label: string }> {
  return [
    { key: '1h', label: tStatic('dashboard.window1h') },
    { key: '6h', label: tStatic('dashboard.window6h') },
    { key: '24h', label: tStatic('dashboard.window24h') },
    { key: '7d', label: tStatic('dashboard.window7d') },
  ];
}
...
function formatDateTime(value: string, locale?: string) {
  return new Date(value).toLocaleString(locale || 'en-US', { hour12: false });
}
...
function TrendLine({ points, color, locale = 'en-US' }: { points: TimeSeriesPoint[]; color: string; locale?: string }) {
        ...
        .map((p, i) => {
          ...
          return (
            <text key={i} x={x} y={height - 4} fill="rgba(148,163,184,0.85)" fontSize="11" textAnchor="middle">
              {new Date(p.timestamp).toLocaleTimeString(locale, { hour: '2-digit', minute: '2-digit', hour12: false })}
            </text>
          );
        })}
    </svg>
  ...
  useEffect(() => {
    ...
    const load = async (isManual = false) => {
      ...
      try {
        ...
        if (!cancelled) {
          toast(tStatic('dashboard.monitoringError'), 'error');
        }
      } finally {
        if (!cancelled) {
          setLoading(false);
  ...
  return (
    <div className="flex-1 overflow-y-auto p-6 space-y-5">
      <div className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
        <div>
          <h2 className="text-lg font-semibold text-gray-100">{t('dashboard.title')}</h2>
          <p className="text-sm text-gray-400">{t('dashboard.desc')}</p>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <span className="text-xs text-gray-500">{t('dashboard.autoRefresh')}</span>
        </div>
      </div>

      {loading ? (
        <div className="h-48 rounded-xl border border-gray-700/40 bg-surface/60 flex items-center justify-center text-sm text-gray-400">{t('dashboard.loading')}</div>
      ) : (
        <>
          ...
          <div className="grid grid-cols-1 xl:grid-cols-3 gap-4">
            <div className="xl:col-span-2 rounded-xl border border-gray-700/30 bg-surface p-4">
              <div className="flex flex-wrap items-center gap-2 mb-3">
                <h3 className="text-sm font-semibold text-gray-200 mr-auto">{t('dashboard.timeSeriesTrend')}</h3>
              ...
              {timeseries && timeseries.points.length > 1 ? (
                <>
                  <TrendLine points={timeseries.points} color={chartColor} locale={locale} />
                  <div className="mt-1 text-xs text-gray-500">
                    {t('dashboard.lastPoint')}: {formatDateTime(timeseries.points[timeseries.points.length - 1].timestamp, locale)}
                  </div>
                </>
              ) : (
                <div className="h-56 flex items-center justify-center text-sm text-gray-500">{t('dashboard.noTimeSeries')}</div>
              )}
            </div>

            <div className="rounded-xl border border-gray-700/30 bg-surface p-4 space-y-4">
              <div>
                <div className="text-xs text-gray-500">{t('dashboard.selectedMetric')}</div>
              ...
              <div className="pt-3 border-t border-gray-700/40">
                <div className="text-xs text-gray-500">{t('dashboard.sampleTime')}</div>
                <div className="text-sm text-gray-200 mt-1">{metrics ? formatDateTime(metrics.sampledAt, locale) : '-'}</div>
              </div>

              <div className="pt-3 border-t border-gray-700/40">
                <div className="text-xs text-gray-500">{t('dashboard.alertSummary')}</div>
          ...
          <div className="rounded-xl border border-gray-700/30 bg-surface p-4">
            <div className="flex flex-wrap items-center gap-2 mb-3">
              <h3 className="text-sm font-semibold text-gray-200 mr-auto">{t('dashboard.alertEvents')}</h3>
            ...
            {visibleAlerts.length === 0 ? (
              <div className="text-sm text-gray-500 py-8 text-center">{t('dashboard.noAlerts')}</div>
            ) : (
                  ...
                  return (
                    <div key={alert.id} className={`rounded-lg border p-3 ${severityClass}`}>
                      <div className="flex flex-wrap items-center gap-2 text-xs mb-1.5">
                        ...
                        <span className="ml-auto text-gray-500">{formatDateTime(alert.lastSeenAt, locale)}</span>
                      </div>
                      ...
                      <div className="text-[11px] text-gray-500 mt-2">
                        {t('dashboard.duration')}: {toDurationLabel(alert.firstSeenAt, alert.resolvedAt ?? alert.lastSeenAt)}
                      </div>
                    </div>
                  );
                })}
